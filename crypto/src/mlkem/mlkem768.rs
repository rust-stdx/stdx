...
mod tests {
    ...
    fn ml_kem_768_cctv_intermediate_vector() {
        ...
        let (k, ct) = crypto_kem_enc_derand::<3, PUBLIC_KEY_SIZE_768, CIPHERTEXT_SIZE_768>(&ML_KEM_768, &ek, &m);
    ...
    fn ml_kem_768_wycheproof_kem() {
        ...
        for group in data["testGroups"].as_array().unwrap() {
            ...
            for test in group["tests"].as_array().unwrap() {
                ...
                if wycheproof_kem_has_invalid_lengths(seed_hex, c_hex, CIPHERTEXT_SIZE_768) {
                    // The key-generation seed is a fixed `[u8; 64]` and the
                    // ciphertext is a fixed `[u8; CIPHERTEXT_SIZE_768]`, so
                    // wrong-length inputs are unrepresentable through the API.
                    assert!(
                        hex::decode(seed_hex).unwrap().len() != 64
                            || hex::decode(c_hex).unwrap().len() != CIPHERTEXT_SIZE_768,
        ...
        eprintln!("wycheproof ML-KEM-768 KEM: {tested} tested, {skipped} skipped (wrong-length inputs)");
    }
    ...
    fn ml_kem_768_wycheproof_encaps() {
        ...
        for group in data["testGroups"].as_array().unwrap() {
            ...
            for test in group["tests"].as_array().unwrap() {
                ...
                if ek_hex.len() != PUBLIC_KEY_SIZE_768 * 2 {
                    ...
                    assert!(
                        PublicKey768::try_from(ek_bytes.as_slice()).is_err(),
        ...
        eprintln!("wycheproof ML-KEM-768 encaps: {tested} tested, {skipped} skipped (wrong-length ek)");
    }
    ...
    fn ml_kem_768_wycheproof_decaps_validation() {
        ...
        for group in data["testGroups"].as_array().unwrap() {
            ...
            for test in group["tests"].as_array().unwrap() {
                ...
                if flags.contains(&"IncorrectDecapsulationKeyLength") {
                    ...
                    assert!(
                        SecretKey768::try_from(dk_bytes.as_slice()).is_err(),
        ...
        eprintln!("wycheproof ML-KEM-768 decaps: {tested} tested, {skipped} skipped (wrong lengths)");
    }
